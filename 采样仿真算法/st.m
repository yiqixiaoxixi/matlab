function [st,t,f] = st(timeseries,fs,paint,minfreq,maxfreq,samplingrate,freqsamplingrate)
...
%			 The rows of STOutput are the frequencies and 
%            the columns are the time values ie each column is
...
factor = 1;%高斯窗的宽度因素为 factor*period
...
if nargin == 3
    minfreq = 0;
    maxfreq = fix(length(timeseries)/2);%fix向0取整
    samplingrate=1;
    freqsamplingrate=1;
elseif nargin==4
...
t = (0:length(timeseries)-1)*samplingrate;%采样点的时间值
spe_nelements =ceil((maxfreq - minfreq+1)/freqsamplingrate);%ceil向上取整
...
if removeedge
    ...
    ind = (0:n-1)';%列向量，值是 0,1,2,3,4,5''''作为线性拟合的横坐标
    r = polyfit(ind,timeseries,2);%最小二乘法曲线拟合,r中为多项式的系数，从小到大排列
    fit = polyval(r,ind);%求拟合后的曲线上的值
...
if analytic_signal%??????????????????????????????????·,??????é????è??¤??????????????°??????????????????????????;
    ...
    % This is correct!
    ts_spe = fft(real(timeseries));%è??????????????????°??????????????????????????;
    h = [1; 2*ones(fix((n-1)/2),1); ones(1-rem(n,2),1); zeros(fix((n-1)/2),1)];%另起一行
    ts_spe(:) = ts_spe.*h(:);
...
tim_est=toc;
vector_fft=[vector_fft,vector_fft];%另起一列
tim_est = tim_est*ceil((maxfreq - minfreq+1)/freqsamplingrate);
if verbose 
    fprintf('Estimated time is %f\n',tim_est);
end

% Preallocate the STOutput matrix
st=zeros(ceil((maxfreq - minfreq+1)/freqsamplingrate),n);%è??????????é??????????,??????????????????é????????????????;
% Compute the mean
